...
vmdf <- psmelt(vt) %>%
  ...
  mutate(sample_id = as_factor(paste(host_genus, host_species, ord, year, sep = "_")),
...
g1 <- ggplot(vmdf, aes(x = sample_id, y = Abundance, fill = DIV, label = DIV)) + 
  geom_col(width = 1) + 
  facet_nested(~ genus_species + ord, 
...
g2 <- ggplot(pmdf, aes(x = year, y = Abundance)) +
    ...
    facet_nested(~ genus_species + ord, scales = "free_x", space = "free_x") +
    scale_x_discrete(expand = c(0, 0)) +
    ...
    theme(legend.position = "bottom", legend.key.size = unit(2, "mm"), legend.title = element_blank(),
          legend.margin = margin(1, 0, 0, 0), legend.box.margin = margin(-10, -10, -10, -10),
