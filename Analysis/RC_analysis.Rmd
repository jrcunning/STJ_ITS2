...
# ggplot(varrdf, aes(x = V1, y = V2, color = host_species)) +
#   geom_point(aes(shape = year)) +
#   geom_path(aes(group = host_id), arrow = arrow(length = unit(0.1, "inches"), type = "open"))
# Phyloseq NMDS
ord <- ordinate(varr, "NMDS", "bray")
plot_ordination(varr, ord, type = "samples") +
  geom_polygon(aes(fill = host_species))


# Calculate beta diversity as distance to centroid of each host species
bd <- betad(phy = varr, group = "host_species")
...
bdsumm <- bdres %>%
  ...
  summarize(meandist = mean(dist_to_centroid),
            se = sd(dist_to_centroid)/sqrt(length(dist_to_centroid)))
