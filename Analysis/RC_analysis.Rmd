...
clades5 <- vmdf %>%
  ...
  ungroup()
Bdom <- clades5 %>% filter(clade == "B")
Cdom <- clades5 %>% filter(clade == "C")
#NMDS
# Look at just Breviolum -dominated corals
varB <- subset_samples(variants, sample_name %in% Bdom$Sample)
...
# Plot NMDS
Bnmds <- as_tibble(varB.nmds.s$data) %>%
  arrange(host_id, year) %>%
  ggplot(aes(x = NMDS1, y = NMDS2, shape = host_species, color = host_species)) +
  geom_point(alpha = 0.5, size = 3) +
  geom_path(aes(group = host_id)) +
  theme_custom()

# Look at just Cladocopium -dominated corals
varC <- subset_samples(variants, sample_name %in% Cdom$Sample)
...
# Plot NMDS
Cnmds <- as_tibble(varC.nmds.s$data) %>%
  arrange(host_id, year) %>%
...
plot_grid(Bnmds, Cnmds, nrow = 1)

bind_rows(varB.nmds.s$data, varC.nmds.s$data, .id = "genus_set") %>%
  arrange(host_id, year) %>%
  ggplot(aes(x = NMDS1, y = NMDS2, color = host_species, shape = host_species)) +
  facet_wrap(~ genus_set, scales = "free") +
  geom_point(alpha = 0.5, size = 6) +
  geom_path(aes(group = host_id), linewidth = 0.1) +
