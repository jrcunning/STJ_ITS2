...
g1 <- ggplot(vmdf, aes(x = sample_id, y = Abundance, fill = DIV, label = DIV)) + 
  geom_col(width = 1) + 
  facet_nested(~ gspp + host_id, labeller = global_labeller,
...
g2 <- ggplot(pmdf, aes(x = year, y = Abundance)) +
    ...
    facet_nested(~ gspp + host_id, scales = "free_x", space = "free_x") +
    scale_x_discrete(expand = c(0, 0)) +
...
ggplot(sc4, aes(x = year2, y = dist, fill = host_species)) +
  facet_wrap(~ host_species, nrow = 1) +
  geom_boxplot()
...
```{r}
varr <- transform_sample_counts(variants, function(x) sqrt(x/sum(x)))    # Square-root transform
#sample_data(varr)[sample_data(varr)$host_id == "59"]
...
bd2 %>% mutate(species1 = factor(species1, levels = levels(brayord$species1))) %>%
