...
vmdf <- psmelt(vt) %>%
  left_join(neword) %>%
  arrange(host_genus, host_species, ord, year, clade, DIV) %>%
...
g1 <- ggplot(vmdf, aes(x = sample_id, y = Abundance, fill = DIV, label = DIV)) + 
  geom_col(width = 1) + 
  facet_nested(~ genus_species + host_id, 
  ...
  scale_fill_manual(values = vpal[vmdf$OTU]) +
  theme(legend.position = "none", panel.background = element_blank(), 
        text = element_text(size = 7), plot.margin = margin(5.5, 5.5, 1, 5.5),
...
pmdf <- psmelt(p) %>%
  left_join(neword) %>%
  arrange(host_genus, host_species, ord, year, clade, its2_type_profile) %>%
  mutate(host_id = as_factor(host_id),
         sample_id = as_factor(paste(host_genus, host_species, host_id, year, sep = "_")),
         genus_species = as_factor(paste(str_sub(host_genus, 1, 1), ". ", host_species, sep = ""))) %>%
...
g2 <- ggplot(pmdf, aes(x = year, y = Abundance)) +
    ...
    facet_nested(~ genus_species + host_id, scales = "free_x", space = "free_x") +
    scale_x_discrete(expand = c(0, 0)) +
...
fig1
ggsave(fig1, filename = "Output/Figure1.png", width = 183, height = 60, units = "mm")
```
...
distmat <- vegan::vegdist(otu_table(varr), method = "bray")
distmat
test <- reshape2::melt(matrix(distmat))[reshape2::melt(upper.tri(distmat))$value,]
distmat

#remotes::install_github("DanielSprockett/reltools")
library(reltools)
test <- make_distance_df(ps = varr, distances = "bray", force_distance_calculations = FALSE, intragroup_only = TRUE,
  variables = "host_id")
sc <- filter(test, same_host_id == "yes")

sc2 <- sc %>%
  mutate(host_id = s1_host_id) %>%
  left_join(sam)

filter(sc2, host_species == "furcata")



ggplot(sc2, aes(x = host_species, y = bray)) + 
  geom_violin() +
  geom_point()

# Plot B-C dist change over time by species
sc3 <- sc2 %>%
  distinct(sample1, sample2, host_species, bray) %>%
  separate(sample1, into = c("site1", "species1", "host_id1", "year1")) %>%
  separate(sample2, into = c("site2", "species2", "host_id2", "year2"))

arrange(sc3, host_id1, year1)


sc4 <- as_tibble(sc3) %>%
  mutate(yearx = if_else(year1 < year2, year1, year2),
         yeary = if_else(year1 < year2, year2, year1)) %>%
  select(host_id = host_id1, host_species,
         year1 = yearx, year2 = yeary, bray) %>%
  filter(year1 == 2017)

# ggplot(sc4, aes(x = year2, y = bray, color = host_species)) +
#   geom_boxplot()

mod <- lmer(bray ~ year2 * host_species + (1|host_id), data = sc4)
hist(residuals(mod))
car::Anova(mod)
...
res2 <- as_tibble(emm) %>%
  full_join(tibble(year2 = "2017", distinct(varrdf, host_species), emmean = 0)) %>%
  filter(host_species != "furcata") %>%
  mutate(host_species = factor(host_species, levels = c("cavernosa", "siderea", "annularis", "natans",
                                                        "labrynthiformis", "franksi", "faveolata", "strigosa")))
...
res2 %>%
  ...
  geom_point(position = position_dodge(width = 0.3)) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), position = position_dodge(width = 0.3), width = 0.1) +
  geom_line(position = position_dodge(width = 0.3)) +
  theme_custom()

```

# Some way of comparing within-colony differences to between-colony differences, to show that there's much more variability among colonies
...
brayord <- bd2 %>%
  ...
  arrange(-medbray) %>%
  mutate(species1 = as_factor(species1))
  
source("https://raw.githubusercontent.com/datavizpyr/data/master/half_flat_violinplot.R")
bd2 %>% mutate(species1 = factor(species1, levels = levels(hi$species1))) %>%
  ggplot(aes(x = group, y = dist)) +
    facet_wrap(~ species1, nrow = 1) +
    geom_boxplot(width = 0.1, outlier.shape = NA, position = position_nudge(x = -0.2), lwd = 0.2) +
    geom_flat_violin(adjust = 1, position = position_nudge(x = 0.15), scale = "width",
                     lwd = 0, fill = "grey72") +
    geom_jitter(width = 0.05, alpha = 0.2, stroke = 0) +
    theme_custom() +
...
bd2 %>%
  ...
  summarize(med = median(dist)) %>%
  pivot_wider(names_from = group, values_from = med) %>%
  ggplot(aes(x = between, y = within, label = species1)) +
...
# between to within ratio
hi <- bd2 %>%
  group_by(species1, group) %>%
  summarize(med = median(dist)) %>%
  pivot_wider(names_from = group, values_from = med) %>%
  mutate(bw = between/within) %>%
  ungroup() %>%
  arrange(-bw) %>%
  mutate(species1 = as_factor(species1))

ggplot(hi, aes(x = within, y = bw)) + geom_point()
```
