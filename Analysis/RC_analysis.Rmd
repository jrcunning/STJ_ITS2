...
# 4 colonies shuffled genera
# how many total colonies sampled more than one year?
pmdf %>%
  count(host_id) %>%
  count(n > 1)
# 59 colonies sampled more than once, and 4 shuffled genera, so 55 did not
...
# changes in background symbionts
vmdf %>%
  group_by(gspp, host_id, year, clade) %>%
  summarize(Abundance = sum(Abundance))  %>%
  filter(Abundance >= 0.01) %>%     # Threshold (as proportion)
  distinct(gspp, host_id, year, clade) %>%
  group_by(gspp, host_id, year) %>%
  summarize(nclades = n_distinct(clade)) %>%
  group_by(gspp, host_id) %>%
  filter(n() > 1) %>%    # only hosts sampled in more than one year
  summarize(nnclades = n_distinct(nclades)) %>%
  arrange(-nnclades) %>%
  print(n = nrow(.))
  
# 32 of 56 (=57%) colonies sampled more than once had a change in number of clades present at > 0.1% abund
# 27 of 56 (=48%) had change in number of clades present at >1%
# 8 of 56 (=14%) had change in number of clades present at >10%

nclad <- vmdf %>%
  group_by(gspp, host_site, host_id, year, clade) %>%
  summarize(Abundance = sum(Abundance)) %>%
  filter(Abundance >= 0.01) %>%   # 1%
  distinct(gspp, host_site, host_id, year, clade) %>%
...
ggplot(nclad, aes(x = year, y = nclades)) +
  geom_jitter(width = 0.1, height = 0.1, alpha = 0.5) +
  #geom_line(aes(group = host_id)) +
  facet_wrap(~ gspp)

mod <- lmerTest::lmer(nclades ~ year * gspp + (1|host_site) + (1|host_id), data = nclad)
hist(residuals(mod))
car::Anova(mod)
emm <- emmeans(mod, c("year", "gspp"))

contrast(emm, "pairwise", by = "gspp")
