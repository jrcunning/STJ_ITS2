...
```{r}
sam0 <- read_csv("Data/0_BL_ITS2_ids.csv") %>%
  clean_names() %>%
  separate(sample_name, into = c("site", "host", "colony", "year"), remove = FALSE)
...
taxnames <- read_tsv(
  #file  = "SymPortal_output/8_STJ_HH_analysis_2018-11-27_14-51-10.886312.DIVs.absolute.txt",
...
plot_both <- function(mycol) {
  s <- subset(sample_data(variants), colony == mycol)
