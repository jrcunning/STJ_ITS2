...
# ITS2 sequencing dataset summary
```{r QC, include = FALSE}
...
# Barplots of sequence variant and profile composition for all samples
```{r}
...
# Get legend for creating multipanel figure
g2leg <- get_legend(g2)
g2leg <- as.ggplot(g2leg) + theme(plot.margin = unit(c(-30, 0, 0, -10), "mm"))
...
ggnet <- as.ggplot(mynet) +
  theme(plot.margin = unit(c(-20, -40, 60, -100), "points"))
```

# Create multipanel Figure 1 with barplots and network plot
```{r}
fig1a <- fig1a + theme(plot.margin = unit(c(5, 0, 0, 0), "mm"))
bottom <- plot_grid(g2leg, ggnet, labels = c("", "b)"), label_x = -0.15) + theme(plot.background = element_rect(fill = "white", color = NA))
fig1 <- plot_grid(fig1a, bottom, nrow = 2, rel_heights = c(0.4, 0.6), labels = c("a)", ""), label_y = 0.9) + 
  theme(plot.margin = unit(c(0, 0, -27, 0), "mm"))
fig1
ggsave(fig1, filename = "Output/Figure1.png", width = 183, height = 120, units = "mm")
```

# NMDS of all sequence variants by symbiont genus
```{r nmds_Fig2}
samples_clades <- vmdf %>%
...
# PERMANOVAs to compare symbionts within a genus across host species
```{r}
...
# Corals that changed dominant symbiont genera or profiles, or number of genera
...
# Changes in dominant profile within a genus
prof_change <- pmdf %>%
  filter(Abundance >= 0.5) %>%
