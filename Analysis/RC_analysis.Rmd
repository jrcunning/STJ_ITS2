...
res2 <- expand_grid(year2 = c("2017", "2018", "2019"),
            host_species = distinct(sc4, host_species)[[1]],
            bray = 0) %>%
  bind_rows(sc4) %>%
  filter(!host_id %in% c("51", "59")) %>%
  group_by(host_species, year2) %>%
  summarize(meanb = mean(bray))
  

res2 %>%
  ggplot(aes(x = year2, y = meanb, color = host_species, group = host_species)) + 
  geom_point(position = position_dodge(width = 0.3)) +
  # geom_errorbar(aes(ymin = response - SE, ymax = response + SE), position = position_dodge(width = 0.3), width = 0.1) +
  geom_line(position = position_dodge(width = 0.3)) +
  theme_custom()
