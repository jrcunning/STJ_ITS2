...
# 2b. symbiont changing over time -- can hierarchical clustering help here by identifying colonies that follow similar trajectories?
```{r}

# get long form data for all variants
as_tibble(data.frame(otu_table(variants)), rownames = "sample_name") %>%
  full_join(select(data.frame(sample_data(variants)), host_id, year, sample_name, host_genus, host_species))
  pivot_longer()
  
  names(sample_data(variants))

longdat <- as_tibble(select(data.frame(sample_data(variants)), 
                            host_id, year, sample_name, host_genus, host_species)) %>%
  full_join(as_tibble(data.frame(otu_table(variants)), rownames = "sample_name")) %>%
  pivot_longer(cols = 6:ncol(.))

# maybe long form data not the way to go. look at hclust on distmat
# Bray-Curtis distance matrix
distmat <- vegan::vegdist(otu_table(varr), method = "euclidean")
...
physeq <- subset_samples(variants, host_id %in% c(81,40,3))
otus <- data.frame(otu_table(physeq), check.names=F)
mcmc <- data.frame(year=sample_data(physeq)$year, host_species=sample_data(physeq)$host_species, 
                   host_id = sample_data(physeq)$host_id, sample=rownames(sample_data(physeq)),
                   otus, check.names=F)
...
smm0 <- OTUsummary(mm, gss, otus=acpass, summ.plot=FALSE)   # Diff & p-vals btw all pairs of factor combinations
smmA <- padjustOTU(smm0)    # Adjust p-values for multiple comparisons
sigs <- signifOTU(smmA, p.cutoff=.9)    # Significant OTUs at FDR level
...
```{r}
# OK let's also try another DE method: ALDEx2 (nope -- too conservative and doesn't pick up significance for lower abundance features
# OK lets do DESeq2
library(DESeq2)
colData = data.frame(sample_data(variants))
colData$host_id <- factor(colData$host_id)
dds <- DESeqDataSetFromMatrix(countData = t(data.frame(otu_table(variants))),
                              colData = colData,
                              design = ~ year)
## Subset Mcav colonies
dds <- dds[, colData(dds)$host_species == "cavernosa"]
### Remove genes counted zero times in subset
dds <- dds[ rowSums(counts(dds)) > 0, ]
### Drop unused factor levels
colData(dds) <- droplevels(colData(dds))
...
taxcounts <- rownames_to_column(taxcounts, var = "variant") %>%
  pivot_longer(-variant, names_to = "sample_name", values_to = "count") %>%
  left_join(dplyr::select(data.frame(sample_data(variants)), "sample_name", "host_id", "year"))
ggplot(taxcounts, aes(x = year, y = count, color = host_id, group = host_id)) +
  facet_wrap(~variant) +
  geom_point() +
  geom_line()
...
#orbicella80,81,40,3 --> nothing sig
out <- deseqDA(host_ids = c(49, 58, 6, 34, 39, 55, 19, 25, 69, 70))
...
# OK now try DESeq for another group of colonies
colData = data.frame(sample_data(variants))
colData$host_id <- factor(colData$host_id)
dds <- DESeqDataSetFromMatrix(countData = t(data.frame(otu_table(variants))),
                              colData = colData,
                              design = ~ year)
## Subset natans 33, 60, 12, 5
dds <- dds[, colData(dds)$host_id %in% c(33, 60, 12, 5)]
### Remove genes counted zero times in subset
dds <- dds[ rowSums(counts(dds)) > 0, ]
### Drop unused factor levels
colData(dds) <- droplevels(colData(dds))
...
dds <- DESeqDataSetFromMatrix(countData = t(data.frame(otu_table(variants))),
                              ...
                              design = ~ year)
## Subset siderea
dds <- dds[, colData(dds)$host_id %in% c(41, 44, 46)]
### Remove genes counted zero times in subset
dds <- dds[ rowSums(counts(dds)) > 0, ]
### Drop unused factor levels
colData(dds) <- droplevels(colData(dds))
