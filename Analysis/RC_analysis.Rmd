...
#Not enough colors for all variants
...
#allcolors <- grDevices::colors()
#set.seed(898)
#vpal <- sample(allcolors, ntaxa(vt))
#vpal <- hue_pal(l = 65)(ntaxa(variants))
#vpal <- setNames(vpal, taxa_names(vt))
...
vmdf <- psmelt(vt) %>%
  arrange(host_genus, host_species, host_id, year, clade, DIV) %>%
  mutate(sample_id = as_factor(paste(host_genus, host_species, host_id, year, sep = "_")),
         DIV = factor(DIV, levels = divord$DIV))
  
g1 <- ggplot(vmdf, aes(x = sample_id, y = Abundance, fill = DIV, label = DIV)) + 
  geom_col(width = 1) + 
  facet_nested(~ host_species + host_id, scales = "free_x", space = "free_x") +
  scale_x_discrete(expand = c(0, 0)) +
